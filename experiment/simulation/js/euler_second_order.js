...
function gety(x1, xn, y1, z1, n) {
    ...
    z[0] = z1;
    for (let i = 1; i <= n; i++) {
        y[i] = y[i - 1] + h * f1(x1, y[i - 1], z[i - 1]);
        z[i] = z[i - 1] + h * g1(x1, y[i - 1], z[i - 1]);
        x1 += h;
    }
    return [y, z];
}
...
function gety2(x1, xn, y1, z1, n) {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        // z[i] = z[i-1]+sz;
        // x1+=h;
        y[i] = y[i - 1] + h * f2(x1, y[i - 1], z[i - 1]);
        z[i] = z[i - 1] + h * g2(x1, y[i - 1], z[i - 1]);
        x1 += h;
    }
    return [y, z];
}
...
function gety3(x1, xn, y1, z1, n) {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        // z[i] = z[i-1]+sz;
        // x1+=h;
        y[i] = y[i - 1] + h * f3(x1, y[i - 1], z[i - 1]);
        z[i] = z[i - 1] + h * g3(x1, y[i - 1], z[i - 1]);
        x1 += h;
    }
    return [y, z];
}
//# sourceMappingURL=euler_second_order.js.map