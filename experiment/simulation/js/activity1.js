...
function activity1() {
    ...
    <div class='divide'>
        <div style='margin-top: 2vw;'>
            <br>
            <h4 class="center-text fs-20px fw-600">ODE: Euler's method for 2<sup>nd</sup> order equation (System of ODE)</h4>
            <p>Objective: </p>
...
function start_act1() {
    ...
    <div class='collapse center-text divide' style='style='margin-top: 2vw; 'width: 80%; margin: auto;' id='tb1-box'>
        ...
        <div id="table-div-act1" style="display:none; margin-top: 2%">
            $$ y_2 = y_1 + h f(x1,y1,z1)  $$
            $$ z_2 = z_1 + h g(x1,y1,z1)  $$
...
function calculate_hxy() {
    ...
    // calculate y
    tbl_data = gety(x1, xn, y1, z1, n_val);
    y05 = tbl_data[0][tbl_data[0].length - 1];
    z05 = tbl_data[1][tbl_data[1].length - 1];
...
function fill_table() {
    ...
    let header = ['x'];
    tbl_data[0].unshift(`y`);
    tbl_data[1].unshift(`z`);
    let n_inputs = [];
    let i = 0;
    for (i = 0; i <= n_val; i++) {
        header.push(`${x_val[i]}`);
    ...
    let tb = new Verify_Rows_Cols_Custom_Fixed(header, tbl_data, [0, 1], [n_inputs, n_inputs], '', div, true, true, exp, 4);
    tb.load_table();
}
function exp() {
